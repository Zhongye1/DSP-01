...
\begin{choicequestion}[单选题]
\question{现有队列 Q 与栈 S，初始时 Q 中的元素依次是 1, 2, 3, 4, 5, 6（1 在队头），S 为空。若仅允许下列 3 种操作：① 出队并输出出队元素；② 出队并将出队元素入栈；③ 出栈并输出出栈元素，则不能得到的输出序列是}{A}
...
\begin{choicequestion}[单选题]
\question{设有一个 $12 \times 12$ 的对称矩阵 $M$ ，将其上三角部分的元素 $m_{i,j}$ （ $1 \leqslant i \leqslant j \leqslant 12$ ）按行优先存入 C 语言的一维数组 N 中，元素 $m_{6,6}$ 在 N 中的下标是}{B}
\choices{50}{51}{55}{66}}
...
\begin{choicequestion}[单选题]
\question{已知二叉排序树如下图所示，元素之间应满足的大小关系是}{C}
...
\begin{choicequestion}[单选题]
\question{下列选项中，不是如下有向图的拓扑序列的是}{D}
...
\begin{choicequestion}[单选题]
\question{高度为 5 的 3 阶 B 树含有的关键字个数至少是}{A}
...
\begin{choicequestion}[问答题]
\question{(12分)拟建设一个光通信骨干网络连通BJ、CS、XA、QD、JN、NJ、TL和WH等8个城市，题42图中无向边上的权值表示两个城市间备选光纤的铺设费用。\newline

\begin{figure}[H]
\centering
\includegraphics[width=0.3\textwidth]{images/02863df12cf706c9940f6fa3b9d932522dbe5798473b0cdae23f1008f1012e15.jpg}
\end{figure}\newline
...
\begin{answersection}
(1) 使用最小生成树算法求解，如Kruskal或Prim算法。由于缺少具体的图结构，无法给出确切的方案和费用。

(2) 可以采用邻接矩阵或邻接表存储结构。求解最小生成树可使用Kruskal算法或Prim算法。

(3) 是否能收到IP分组取决于TL到BJ的最短路径长度是否小于等于5。如果在最经济方案中TL到BJ的路径长度不超过5跳，则H2可以收到分组；否则，TTL减到0时分组会被丢弃，H2收不到分组。
\end{answersection}

\begin{analysissection}
最小生成树问题要求找到连接所有顶点的边的集合，使得总权重最小。

(1) 最小生成树方案：由于没有图的具体结构，一般方法是使用Kruskal或Prim算法，选择权重最小的边，确保不形成环路，直到连接所有顶点。

(2) 图的存储结构：
- 邻接矩阵：适合稠密图，空间复杂度O(V²)
- 邻接表：适合稀疏图，空间复杂度O(V+E)
最小生成树算法：
- Kruskal算法：基于边的排序，使用并查集
- Prim算法：基于顶点的扩展

(3) TTL（Time To Live）是IP分组中的字段，每经过一个路由器就减1，当TTL为0时分组被丢弃。如果TL到BJ的最短路径跳数≤5，则分组可以到达；否则在途中被丢弃。
\end{analysissection}
\end{choicequestion}
