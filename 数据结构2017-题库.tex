...
\begin{choicequestion}[单选题]
\question{适用于压缩存储稀疏矩阵的两种存储结构是}{A}
...
\begin{choicequestion}[单选题]
\question{要使一棵非空二叉树的先序序列与中序序列相同，其所有非叶结点须满足的条件是}{B}
...
\begin{choicequestion}[单选题]
\question{已知一棵二叉树的树形如右图所示，其后序序列为e,a,c,b,d,g,f，树中与结点a同层的结点是}{B}
...
\begin{choicequestion}[单选题]
\question{已知字符集 $\{a, b, c, d, e, f, g, h\}$ ，若各字符的哈夫曼编码依次是0100,10,0000,0101,001,011,11,0001，则编码序列0100011001001011110101 的译码结果是}{C}
...
\begin{choicequestion}[单选题]
\question{已知无向图G含有16条边，其中度为4的顶点个数为3，度为3的顶点个数为4，其他顶点的度均小于3。图G所含的顶点个数至少是}{B}
\choices{10}{11}{13}{15}}
...
\begin{choicequestion}[单选题]
\question{下列二叉树中，可能成为折半查找判定树（不含外部结点）的是}{A}
