...
\begin{choicequestion}[单选题]
\question{现有非空双向链表 L, 其结点结构为: prev data next, prev 是指向直接前驱结点的指针, next 是指向后继结点的指针。若要在 L 中指针 p 所指向的节点 (非尾结点) 之后插入指针 s 指向的新结点, 则在执行了语句序列 "s -> next = p -> next; p -> next = s" 后, 下列语句序列中还需要执行的是 ( )。}
...
\begin{choicequestion}[单选题]
\question{已知一棵二叉树的树形如右图所示，若其后序遍历序列为 f, d, b, e, c, a，则其先（前）序遍历序列是（）。}
